...
# main_url = 'https://eurasiantimes.com/'
class Eurasian_News():
    """
    ...
    def News_Title(self, soup):
        try:
            ...
            if title:
                ...
                title = soup.find('h2', class_=['tdb-block-inner td-fix-index', 'text-7.5', 'font-bold', 'font-playfair-display', 'mt-1', 'pb-3', 'border-b', 'border-gray-300', 'border-solid',
    ...
    def Scrap_Latest_Links(self, url):
        ...
        if html_document is not None:
            ...
            results = soup.find_all('div',
                                    class_=['td_block_wrap td_flex_block_1 tdi_70 td-pb-border-top td_block_template_4 td_ajax_preloading_preload td_flex_block', ])  # 'full-light-container'
            # results_links = [i for i in results if i is not None]
            for div in results:
                links = div.findAll('a', href=True)
                # a_tag.append(links)
    ...
    def Scrap_Eurasian_News(self, article_urls, name, **kwargs):
        ...
        print("-------------------------------Scrapping Eurasian News Paper-------------------------------")
        i = 0
    ...
    def scrap_latest_news(self, latest_url,  **kwargs):
        ...
        df_latest = self.Scrap_Eurasian_News(article_urls, 'Eurasiantimes Latest')
    ...
    def scrap_national_news(self,  latest_url, **kwargs):
        ...
        df_national = self.Scrap_Eurasian_News(article_urls, 'Eurasiantimes AsiaPacific')
    ...
    def scrap_world_news(self,  latest_url, **kwargs):
        ...
        df_world = self.Scrap_Eurasian_News(article_urls, 'Eurasiantimes International')
        # df_national = pd.DataFrame.from_dict(df_news)
        return df_world


if __name__ == "__main__":
    print("-------------------------------Scraping Eurasiantimes News -------------------------------")
    scrap = Eurasian_News()
    latest = 'https://eurasiantimes.com/'
    national = 'https://eurasiantimes.com/category/asia-pacific/'
    world = 'https://eurasiantimes.com/category/world/'
