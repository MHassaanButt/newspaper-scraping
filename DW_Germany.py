...
class DW():
    ...
    def News_Time(self, soup):
        try:
            ...
            if "hour" in time_scraped:
                ...
                news_time = datetime.strptime(time_now, format) - \
    ...
    def Scrap_Latest_Links(self, url):
        ...
        links = []
        news_feed_results = soup.find(
            "section", id=["news-feed-container"]).find_all("article")
        # results2 = soup.find_all('div', attrs={'class':'l-col l-col--8'})
        for result in news_feed_results:
            if result.find('h3', attrs={'class', 'gc__title'}).a['href']:
                link = result.find(
                    'h3', attrs={'class', 'gc__title'}).a['href']
                link = main_url+link
                links.append(link)
                print('Link-----------', link)
            else:
    ...
    def Scrap_National_Links(self, url):
        ...
        # results2 = soup.find_all('div', attrs={'class':'l-col l-col--8'})
        for result in news_feed_results:
            link = result.find('h3', attrs={'class', 'gc__title'}).a['href']
            link = main_url+link
            links.append(link)
        return links

    def Scrap_World_Links(self, world_links):
        main_url = 'https://www.aljazeera.com'
        links = []
        for url in world_links:
            html_document = self.HTML_Document(url)
            soup = self.B_Soup(html_document)
            news_feed_results = soup.find(
                "section", id=["news-feed-container", ]).find_all("article")
    ...
    def Scrap_DW(self, article_urls, name, **kwargs):
        ...
        df.to_csv('DW_scraped.csv', index=False)
    ...
    def scrap_latest_news(self, latest_url,  **kwargs):
        ...
        df_latest = self.Scrap_DW(article_urls, 'Al-Jazeera Latest')
        return df_latest
    ...
    def scrap_national_news(self,  latest_url, **kwargs):
        ...
        df_national = self.Scrap_DW(article_urls, 'Al-Jazeera National')
        return df_national
    ...
    def scrap_world_news(self,  latest_url, **kwargs):
        ...
        df_national = self.Scrap_DW(article_urls, 'Al-Jazeera International')
        return df_national
...
if __name__ == "__main__":
    ...
    world = ['https://www.dw.com/en/africa/s-12756', 'https://www.dw.com/en/latin-america/s-58267484',
             'https://www.dw.com/en/asia/s-12758', 'https://www.dw.com/en/middle-east/s-14207', 'https://www.dw.com/en/europe/s-1433', 
    ...
    # df_news = pd.DataFrame.from_dict(al_jazeera_latest)
    # df_news.to_csv('national_scraped.csv', index=False)
    # df_news.head()
